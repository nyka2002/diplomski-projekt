/**
 * Index.hr Oglasi Scraper
 *
 * Scrapes real estate listings from Index.hr/oglasi
 */

import { Page } from 'playwright';
import { BaseScraper } from '../../base';
import { IndexOglasiParser } from './parser';
...
export class IndexOglasiScraper extends BaseScraper {
  private parser: IndexOglasiParser;
  ...
  constructor(listingType: ListingType, propertyType: PropertyType = 'apartment') {
    super({
      source: 'index-oglasi',
      baseUrl: SCRAPING_CONFIG.sources.indexOglasi.baseUrl,
      listingType,
      propertyType,
      maxPages: SCRAPING_CONFIG.sources.indexOglasi.maxPagesPerScrape,
    });

    this.parser = new IndexOglasiParser();
  ...
  getListingPageUrl(page: number): string {
    const { baseUrl } = SCRAPING_CONFIG.sources.indexOglasi;
    const path = this.getPathForType();

    const url = new URL(`${baseUrl}${path}`);

    // Add property type filter
    if (this.config.propertyType === 'apartment') {
      url.searchParams.set('tip', 'stan');
    } else if (this.config.propertyType === 'house') {
      url.searchParams.set('tip', 'kuca');
    }

    if (page > 1) {
      url.searchParams.set('stranica', String(page));
    }
  ...
  async parseDetailPage(page: Page, url: string): Promise<RawListingData | null> {
    try {
      ...
      console.error(`Failed to parse Index detail page ${url}:`, (error as Error).message);
      return null;
    }
  }
  ...
  /**
   * Get the URL path for the current listing type
   */
  private getPathForType(): string {
    const { indexOglasi } = SCRAPING_CONFIG.sources;
    const { listingType } = this.config;

    return listingType === 'rent' ? indexOglasi.rentPath : indexOglasi.salePath;
  }
}
